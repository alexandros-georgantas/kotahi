...
describe('notifications tests', () => {
    ...
    context('sent notification via control panel', () => {
      ...
      it.skip('sent notification to unregistered user', () => {
        ...
        cy.get('[class*="TextInput__StyledInput"]:last').type('First task for unregistered user')
        cy.get('[data-testid="Assignee_select"]').type('Unregistered User{enter}')
        cy.get('[data-cy="new-user-email"]').focus().type('uku.sidorela@gmail.com')
      ...
      it('sent notification 3 notifications via task details modal', () => {
        ...
        cy.wait(200)
        cy.get('[class*=SecondaryActionButton__LabelOnlySpan]').click({ force: true})
        ...
        cy.get('[class*=SecondaryActionButton__LabelOnlySpan]:last').click({ force: true})
        cy.get('[data-testid="Recipient_select"]:last').type('Sherry Crofoot{enter}')
        // eslint-disable-next-line cypress/no-unnecessary-waiting
        cy.wait(20)
        cy.get('[data-testid="Notification_email_select"]:last').click()
